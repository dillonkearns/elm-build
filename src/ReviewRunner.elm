...
reviewRunnerHelperSource =
    String.join "\n"
        ...
        , "runReview modules ="
        , "    modules"
        , "        |> List.concatMap (\\mod -> checkSource mod.path mod.source)"
        ...
        , "        indexedLines = List.indexedMap (\\i line -> (i + 1, line)) lines"
        , "    in"
        , "    indexedLines"
        , "        |> List.concatMap (\\(lineNum, line) ->"
        , "            let"
        , "                trimmed = String.trim line"
        , "            in"
        , "            if String.contains \"Debug.log\" trimmed then"
        , "                case findColumn \"Debug.log\" line of"
        , "                    Just col ->"
        , "                        [ { ruleName = \"NoDebug.Log\", filePath = path, line = lineNum, column = col, message = \"Remove the use of Debug.log\" } ]"
        , "                    Nothing -> []"
        , "            else []"
        , "        )"
        , ""
        , "findColumn needle haystack ="
        , "    case String.indexes needle haystack of"
        , "        first :: _ -> Just (first + 1)"
        , "        [] -> Nothing"
        , ""
        ...
        , "            [ { ruleName = \"NoDebug.Log\", filePath = path, line = range.start.row, column = range.start.column, message = \"Remove the use of Debug.log\" } ]"
